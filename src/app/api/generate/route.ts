...
const FALLBACK_CONTENT = (name: string, cuisine: string, description: string): RestaurantContent => ({
...
【店舗情報】店名:${name} ジャンル:${cuisine} 住所:${address} 説明:${description} 価格帯:${priceRange} 評価:${rating}

以下のJSON形式で出力（コードブロック・説明文不要）:
{"tagline":"キャッチコピー20字以内","conceptTitle":"見出し15字以内","conceptText":"150〜200文字のこだわり説明","pillars":[{"icon":"絵文字","title":"タイトル","text":"説明40〜60字"}×3],"menu":[{"name":"料理名","desc":"説明40〜60字","price":"数字のみ","badge":"バッジ","imgKeyword":"英語ワード"}×6],"colorPrimary":"HEX","colorAccent":"HEX","colorBg":"HEX","colorText":"HEX","fontHeading":"Google Fontsフォント名","fontBody":"Google Fontsフォント名"}
`
    ...
    try {
      ...
      content = FALLBACK_CONTENT(name, cuisine, description)
    }

    const meta: RestaurantMeta = {
      name, cuisine, address, phone, hours, priceRange, seats, access, rating,
      tabelogUrl: tabelogUrl || '', googleMapsUrl: googleMapsUrl || '',
    ...
    const restaurantData = {
      ...
      name, cuisine, address, phone, hours, description, priceRange, seats, access, rating,
      googleMapsUrl: googleMapsUrl || '', tabelogUrl: tabelogUrl || '',
      imageUrls, uploadedHeroImg: '', uploadedMenuImgs: [], uploadedGalleryImgs: [],
      contentJson: JSON.stringify(content),
      metaJson: JSON.stringify(meta),
      generatedHtml, status: 'demo',
      createdAt: new Date().toISOString(),
    }

    await saveRestaurant(slug, restaurantData)

    return NextResponse.json({ slug, password: plainPassword })
  } catch (e) {
